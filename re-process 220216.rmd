...
Precision on the scale corrections were +/ 0.078 permil (d13C) and +/- 0.603 permil (d18O). All analyses (samples = sa, standards = st) culled are listed below (see output file for details on culling):
- 23325 - The cap was skewed to the side and loose  

*consider removing (but not removed at this point):*
-23358 -> CU YULE with a puckered cap & small peaks. No apparent atm 
-23355 -> IAEA C2 with a mass of 106. The peaks look totally fine. So it was probably just a bad measurement.  
...
library(stringr)
  #if loading this package for the first time, be sure to install the "devtools" package first, then install the isoprocessCUBES package: devtools::install_github("CUBESSIL/isoprocessCUBES")
```
...
data <- 
  raw.data %>% 
  filter(Nr. > 5) %>%
  filter(file_datetime > "2022-02-16 00:00:00") %>% 
...
IAEA C2, linearity suite, mass 155 
O75_whole, mass 118 
```{r reorder observations}
...
samples <- samples %>% 
  mutate(
    SoilSample = str_sub(Identifier1, 1,4) )
  
  

plt1 <- samples %>% 
  filter(Do_not_use != "yes") %>% 
  ggplot()+
  aes(x = d18O.scale, y = d13C.scale, color = SoilSample)+
  geom_point()+
  geom_errorbarh(aes(xmin = d18O.scale - d18O.error.S, xmax = d18O.scale + d18O.error.S )) +
  geom_errorbar((aes(ymin = d13C.scale - d13C.error.S, ymax = d13C.scale + d13C.error.S)))
...
plt2 <- samples %>% 
  filter(Do_not_use != "yes") %>% 
  ggplot()+
  aes(x = d18O.scale, y = d13C.scale, color = Identifier1)+
  geom_point()+
  geom_errorbarh(aes(xmin = d18O.scale - d18O.error.S, xmax = d18O.scale + d18O.error.S )) +
  geom_errorbar((aes(ymin = d13C.scale - d13C.error.S, ymax = d13C.scale + d13C.error.S)))
