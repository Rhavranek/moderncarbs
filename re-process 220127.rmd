...
I used the raw data before the scale correction. I considered using linearity becuase it minimzed uncertainty on the correcting standard - but there was little to no linearity slope and so that felt "false".


Examples:
22992 - large standard deviation in both oxygen and carbon - consider removing 
...
The two vials that never got stabbed were: 
CU YULE, analysis 22992, mass = 112

22982 - 22991 are fine 
22993 - 23023 scootch up 1 


S75 definitely never ran, and I don't think S100 did either. 


```{r}
data2 <- data

for (ID in 23023:22992){
  ID2 <- ID+1
...
data2$d18O.raw <- (data2$d18O.raw.SMOW - 41.43)/1.04143 #d18O output is CO2 SMOW, so need to use the CO2 SMOW to min PDB conversion

stdev <- gather(filter(data2, num.peaks > 1, d18O.sd < 100), key = "isotope",
...
culled.data <- bind_rows(culled.data, filter(data2, d18O.sd > d18O.sd.cutoff))
wo.culled <- filter(data2, d18O.sd < d18O.sd.cutoff)

stds1<- filter(data2, type != "sample" & d18O.sd < d18O.sd.cutoff)
...
stds.to.cull <- c(22992) # #Analysis #s with yield problem and/or significant outlier in d13C or d18O, including samples smaller than linearity range that weren't caught by other culling steps
...
yield.all <- ggplot(data2, aes(x = mass, y = area44, fill = type, label = Analysis)) +
...
more.files <- c(data2$Analysis[11])

plot.more <- filter(data2, Analysis %in% more.files)
...
data2$PercentCO3 <- ((yield.slope * data2$area44 + yield.intercept)/data2$mass *100)

data2$target.wgt.ug <-  90/(data2$PercentCO3/100)
```
...
data2$d13C.offset <- data2$d13C.raw + (filter(C.stds.table, std.type == "corr.std")$C.acc - offsetC.mean)
...
data2$d13C.drift <- data2$d13C.raw + (filter(C.stds.table, std.type == "corr.std")$C.acc - (drift.slopeC * data2$row + drift.interC))
...
data2$d13C.lin <- data2$d13C.raw +  (filter(C.stds.table, std.type == "corr.std")$C.acc - (lin.slopeC * data2$inv.area44 + lin.interC))
...
lindriftC <- merge(driftC, data2[c("row", "d13C.lin")], by.x = "row", by.y = "row", all.x = TRUE, all.y = FALSE, sort = FALSE)
...
data2$d13C.lindrift <- data2$d13C.lin +  (filter(C.stds.table, std.type == "corr.std")$C.acc - (lindrift.slopeC * data2$row + lindrift.interC))
...
# safety checks
if (!col_in_data %in% names(data2)) stop("this column does not exist in data or stds table: ", col_in_data, call. = FALSE)
...
C.scale.all <- 
  ...
  geom_point(data = filter(data2, d18O.sd < d18O.sd.cutoff), aes_string(x = col_in_data, y = "d13C.scale"), shape = 23, fill = "red", size = 2) +
...
#apply offset correction to whole dataset
data2$d18O.offset <- data2$d18O.raw +  (filter(O.stds.table, std.type == "corr.std")$O.acc - offsetO.mean)
...
data2$d18O.drift <- data2$d18O.raw + (filter(O.stds.table, std.type == "corr.std")$O.acc - (drift.slopeO * data2$row + drift.interO))
...
data2$d18O.lin <- data2$d18O.raw +  (filter(O.stds.table, std.type == "corr.std")$O.acc - (lin.slopeO * data2$inv.area44 + lin.interO))
...
lindriftO <- merge(driftO, data2[c("row", "d18O.lin")], by.x = "row", by.y = "row", all.x = TRUE, all.y = FALSE, sort = FALSE)
...
data2$d18O.lindrift <- data2$d18O.lin +  (filter(O.stds.table, std.type == "corr.std")$O.acc - (lindrift.slopeO * data2$row + lindrift.interO))
...
# replace the character string here with the correction column you want to use
col_for_scale_O <- "lin"  # options to substitute in here are: "raw" or "offset" or "drift" or  "lin" or  "lindrift"
...
O.scale.all <- 
  ...
  geom_smooth(method = "lm", color = "blue") +
  geom_point(data = filter(data2, d18O.sd < d18O.sd.cutoff), aes_string(x = col_in_data, y = "d18O.scale"), shape = 23, fill = "red", size = 2) +
  geom_point(shape = 21, fill = "blue", size = 2) +
  geom_text(
...
data2$d13C_scale_input <- col_for_scale_C
data2$d18O_scale_input <- col_for_scale_O
data2$Run <- Run
...
```{r save.data}
add_ws_with_data <- function(wb, sheet, data2) {
  addWorksheet(wb, sheet)
  writeData(wb, sheet=sheet, data2)
  return(wb)
}
...
saveWorkbook(wb, paste0(session, Run, "_corrected_data.xlsx"), overwrite = TRUE)
```
