---
title: "Modern carbonate samples"
subtitle: "Run 2"
analyst: "Rachel Havranek"
run date: "02/16/2022"
output:
  html_document: 
    df_print: paged
...
Run description:
* on the sequence I didn't have 11 or 43 in the AS sample column, so everything after 10 was **offset**
...
Examples:
-23325, IAEA C2, mass 47 -> the cap was askew when I pulled it out of the gasbench. Does fine in carbon space, but is an outlier (-5 per mil in oxygen space). Had a surprisingly good yield. had large atm peaks 
...
raw.data <- dxf_data %>% 
  ...
  filter(file_datetime > "2022-02-05")

data2<-data
for (ID in c(18364:18359)){
  ID2 <- ID-1
  data[ data2$Analysis == ID , "Identifier1"] <- data[ data$Analysis == ID2 , "Identifier1"]
  data[ data2$Analysis == ID , "mass"] <- data[ data$Analysis == ID2 , "mass"]
  data[ data2$Analysis == ID , "carousel"] <- data[ data$Analysis == ID2 , "carousel"]
  data[ data2$Analysis == ID , "type"] <- data[ data$Analysis == ID2 , "type"]
  }
...
#raw.data[  raw.data$Analysis == 23328, "mass"] <- 208
#raw.data[  raw.data$Analysis == 9101, "Identifier1"] <- "NBS18"
#raw.data[  raw.data$Analysis == 23329, "mass"] <- 155
```
...
Standardsforthisrun <- tibble(
   std.type = as.ordered(c("corr.std", "dis2.std", "dis.std", "mon.std")),
   Sample.ID = as.ordered(c(corr.std, dis2.std, dis.std, mon.std)),
   std.name = as.ordered(c(corr.std, dis2.std, dis.std, mon.std)))  %>% 
    inner_join(Standards, by = "Sample.ID") %>% 
      select(std.type, std.name, d13C, d18O )
  
C.stds.table <- Standardsforthisrun %>% 
   select(std.type, std.name, d13C) %>% 
    rename(C.acc = d13C)
 
O.stds.table <- Standardsforthisrun %>% 
  select(!"d13C") %>% 
  rename(O.acc = d18O)
...
C.acc.corr <-  C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc)
O.acc.corr <-  O.stds.table %>% filter(std.name == corr.std) %>% select(O.acc)
```
...
```
*WHAT TO LOOK FOR*
- stdev vs. area44 plots show a link between how much gas there was and how well it ran. 
- if its standards that didn't run well in addition to samples - then need to chase down another problem
- oxygen lately isn't running as well 
- stop and do maintenance on gasbench? 
- histogram looking for normal distribution around a 'normal' stdev
- go back and look at vials right after the run (next day) -> can help pinpoint yield problems. 

*NOTES*
1. Oxygen is far more symmetrical than carbon.
1. Oxygen's SD is about the same as carbon's - that's good!
1. There's still a strong population of small samples 
...
* this cut off got rid of the oxygen outliers, but not the one carbon outlier (st dev = 0.35)
* the stdev median is 0.12 for carbon and 0.13 for oxygen (back to being pretty much the same)
...
stds.to.cull <- c(23325, 23333) # #Analysis #s with yield problem and/or significant outlier in d13C or d18O, including samples smaller than linearity range that weren't caught by other culling steps
...
std_values <- stds %>% 
  ggplot()+
  aes(x = d18O.raw, y = d13C.raw, fill = Identifier1, size =2, text = Analysis)+
  geom_point()

ggplotly(std_values, dynamicTicks = TRUE)
...
offsetC$d13C.offset <- offsetC$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - offsetC.mean)
...
data$d13C.offset <- data$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - offsetC.mean)
stds$d13C.offset <- stds$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - offsetC.mean)
...
#drift check
driftC$d13C.drift<- driftC$d13C.raw + (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (drift.slopeC * driftC$row + drift.interC))
...
data$d13C.drift <- data$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (drift.slopeC * data$row + drift.interC))
stds$d13C.drift <- stds$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (drift.slopeC * stds$row + drift.interC))
...
#linearity check
linC$d13C.lin<-linC$d13C.raw + (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lin.slopeC * linC$inv.area44 + lin.interC))
...
data$d13C.lin <- data$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lin.slopeC * data$inv.area44 + lin.interC))
stds$d13C.lin <- stds$d13C.raw +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lin.slopeC * stds$inv.area44 + lin.interC))
...
#drift check
lindriftC$d13C.lindrift<- lindriftC$d13C.lin + (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lindrift.slopeC * lindriftC$row + lindrift.interC))
...
data$d13C.lindrift <- data$d13C.lin +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lindrift.slopeC * data$row + lindrift.interC))
stds$d13C.lindrift <- stds$d13C.lin +  (C.stds.table %>% filter(std.name == corr.std) %>% select(C.acc) - (lindrift.slopeC * stds$row + lindrift.interC))
...
keydata <- data %>% 
  ungroup() %>%  
    filter(!(Identifier1 %in% c(corr.std, mon.std, dis.std, dis2.std))) %>% 
